...
func follow_path():
	while path.size() > 0 and path[0].distance_to(character.global_transform.origin) < 0.1:
		path.pop_front()
	if path.size() > 0:
		character.state.move_direction = path[0] - character.global_transform.origin
		character.state.move_direction *= speed_multiplier
		var facing = character.state.face_direction
		facing.y = 0
